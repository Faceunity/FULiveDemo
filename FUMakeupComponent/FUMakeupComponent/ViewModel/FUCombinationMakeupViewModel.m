...
- (void)bindCombinationMakeupWithBundleName:(NSString *)bundleName {
    NSString *path = [FUMakeupBundle() pathForResource:bundleName ofType:@"bundle"];
...
- (NSArray<FUCombinationMakeupModel *> *)combinationMakeups {
    if (!_combinationMakeups) {
        NSString *path = [FUMakeupBundle() pathForResource:@"combination_makeups" ofType:@"json"];
        ...
        for (NSDictionary *dictionary in data) {
            ...
            if (![model.name isEqualToString:@"卸妆"]) {
                // 解析对应组合妆的json文件
                NSString *combinationPath = [FUMakeupBundle() pathForResource:model.bundleName ofType:@"json"];
