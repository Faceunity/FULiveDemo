...
@property (nonatomic, strong) NSMutableArray <FUWholeAvatarModel *>*dataArray;
@property (weak, nonatomic) IBOutlet UIView *bottomView;
@property (weak, nonatomic) IBOutlet UILabel *mTitleLabel;
@end

@implementation FUAvatarsController
...
//    self.noItemLabel.hidden = [FUManager shareManager].myStickers.count != 0;
    ...
    for (int i = 0; i  < [FUAvatarPresenter shareManager].wholeAvatarArray.count ; i ++ ) {
        if (i  < defaultAvatarNum) {
            continue;
        }
         [_dataArray addObject:[FUAvatarPresenter shareManager].wholeAvatarArray[i]];
    }
...
- (void)certainDeleteAction {
    ...
    for (FUWholeAvatarModel *obj in temArray) {
        [[FUAvatarPresenter shareManager].wholeAvatarArray removeObject:obj];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    FUWholeAvatarModel *modle = _dataArray[indexPath.row];
    cell.itemImage.image = modle.image;
