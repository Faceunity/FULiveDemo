...
-(void)saveBtnClick:(UIButton *)btn{
    ...
    int count  = (int)[FUYItuSaveManager shareManager].dataDataArray.count;
    NSString *midStr = [NSString stringWithFormat:@"yitu%d",count + 1];
    [FUYItuSaveManager saveImg:self.imageView.image withVideoMid:midStr];
    ...
    for (UIView *view in self.view.subviews) {
        if ([view isKindOfClass:[FUAdjustImageView class]]) {
            ...
            [group_type addObject:@(view0.type)];
            if (view0.miniPoints.count) {//通过精细调整
                for (NSNumber *num in view0.miniPoints) {
                    [group_points addObject:num];
                }
            }else{
                NSArray *points =  [view0 getConvertPointsToView:self.view];//相对imageView坐标数组
                NSArray *imagePoint = [self coordintePhoneToImage:points];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2.0 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self.navigationController popViewControllerAnimated:YES];
        if (self.returnBlock) {
            self.returnBlock((int)[FUYItuSaveManager shareManager].dataDataArray.count - 1);
        }
    });
}
/* 调整 */
-(void)miniAdjusetBtnClick:(UIButton *)btn{
    ...
    vc.pointsArray = [[self coordintePhoneToImage:points] mutableCopy];
    CGPoint imageC = [_curAdjustView getConvertCenterImage:self.view];
    CGPoint imageCenter = [self coordintePhoneToPoint:imageC];
    //归一
    ...
    vc.adjustedLandmarksSuccessBlock = ^(NSMutableArray *array) {
        _curAdjustView.miniPoints = array;
        
//        CGPoint newPoint = CGPointMake(0, 0);
//        for (int i = 0; i < array.count; i++) {
//            double xy = [array[i] doubleValue];
//            if (i % 2 && i != 0) {
//                newPoint.y = xy;
...
/* 手机坐标点转图片坐标 */
-(NSArray *)coordintePhoneToImage:(NSArray *)points{
...
-(CGPoint )coordintePhoneToPoint:(CGPoint )point{
    CGRect iamgeAspectRect = AVMakeRectWithAspectRatioInsideRect(_imageView.image.size,_imageView.bounds);
    double magrinX =  iamgeAspectRect.origin.x;//图片在ImageView中的X空隙
    double magrinY =  iamgeAspectRect.origin.y;//图片在ImageView中的Y空隙
    double screenW = [UIScreen mainScreen].bounds.size.width;
...
-(void)adjustImageViewAddOne:(FUAdjustImageView *)view{
    [self yituAddItemView:view.type];
}
