...
@interface FUQualityStickerViewModel ()
...
@implementation FUQualityStickerViewModel
...
- (void)downloadItem:(FUQualityStickerModel *)sticker completion:(void (^)(NSError * _Nullable))completion {
    [self.downloadQueue addOperationWithBlock:^{
        ...
        [FUQualityStickerHelper downloadItem:sticker completion:^(NSError * _Nullable error) {
...
- (void)loadAvatarItem:(FUQualityStickerModel *)stickerModel {
    [[FURenderKit shareRenderKit] addScene:self.scene completion:^(BOOL success) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            self.avatar.position = FUPositionMake(25.2, FUDeviceIsiPhoneXStyle() ? (56.14 - 50.0*24.0/72.0) : 56.14, -537.94);
...
- (void)loadCommonItem:(FUQualityStickerModel *)stickerModel {
...
- (FUScene *)scene {
    if (!_scene) {
        ...
        _scene = [[FUScene alloc] initWithControllerConfigPath:controllerConfigPath];
