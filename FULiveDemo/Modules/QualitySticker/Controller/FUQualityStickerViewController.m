...
- (void)setupContentView {
    ...
    [self updateBottomConstraintsOfCaptureButton:FUHeightIncludeBottomSafeArea(231) + 10 animated:NO];
}
...
- (void)changeStickerViewStatus:(BOOL)isShow {
    ...
    [self updateBottomConstraintsOfCaptureButton:isShow ? (FUHeightIncludeBottomSafeArea(231) + 10) : (FUHeightIncludeBottomSafeArea(49) + 10) animated:YES];
}
...
- (void)stickerPageController:(FUQualityStickerPageController *)controller shouldLoadSticker:(FUQualityStickerModel *)sticker {
    if ([sticker.tag isEqualToString:self.selectedSticker.tag] && [sticker.itemId isEqualToString:self.selectedSticker.itemId]) {
        ...
        if (sticker.isSingle) {
            ...
            // 最多四个人脸
            self.viewModel.maxHumanNumber = 4;
        }
...
- (UIView *)bottom {
    if (!_bottom) {
        _bottom = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.frame), FUHeightIncludeBottomSafeArea(49))];
