...
- (void)viewDidLoad {
    ...
    [arr addObjectsFromArray:self.segmentManager.items];
    _list = [NSMutableArray arrayWithArray:arr];
    self.itemsView.items = _list;

    /* 初始状态 */
    NSInteger selectedIndex = _list.count > 0 ? 2 : 0;
    if ([self.segmentManager isHaveCustItem]) {
        selectedIndex = 3;//人像描边
    }
    self.itemsView.selectedIndex = selectedIndex;
    
    [self itemsView:self.itemsView didSelectItemAtIndex:selectedIndex];
    
    self.segmentManager.selectedItem = self.list[selectedIndex];
...
-(void)viewWillAppear:(BOOL)animated {
    ...
    if (self.itemsView.selectedIndex < 0) {
        self.itemsView.selectedIndex = [self.list indexOfObject:self.segmentManager.selectedItem];
    }else {
        //自定义背景并且背景
        if ([self.itemsView.items[self.itemsView.selectedIndex] isEqualToString:CUSTOMBG]) {
...
-(void)setupView {
    self.itemsView = [[FUItemsView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.view.bounds), 84)];
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                if (finished) {
                    ...
                    if (model.type == FUBGSaveModelTypeVideo) {
                        ...
                        if (!image) {
                            image = [FUImageHelper getPreviewImageWithVideoURL:model.url];
                        }
                        if (image) {
                            ...
                            weak.itemsView.items = _list;
                            weak.itemsView.selectedIndex = 2;
                            
                            //移除缓存的第一帧图片
                            [weak removeCacheImage: CUSTOMBG];
                            //刷新默认的outline 效果
                            [weak itemsView:weak.itemsView didSelectItemAtIndex:2];
                        }
                    } else {
...
-(void)didClickSelPhoto{
    ...
    vc.didCancel = ^{
        _itemsView.selectedIndex = [self.list indexOfObject:self.segmentManager.selectedItem];
...
- (void)selectImage:(UIImage *)image {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        self.itemsView.items = _list;
        self.itemsView.selectedIndex = 2;
        [self itemsView:self.itemsView didSelectItemAtIndex:2];
    });
}
